...
import { normalizeScheduleModuleKey } from "../../../utils/schedule/scheduleColors.js";
...
const SCHEDULE_DEPARTMENT_FILTERS = ["全部", "project", "aigc", "design", "threeD", "motion", "post", "项目管理", "AIGC", "美术设计", "三维动态", "动效设计", "视效包装"];
const SCHEDULE_DEPARTMENT_FILTER_LABELS = new Set(["项目管理", "AIGC", "美术设计", "三维动态", "动效设计", "视效包装"]);
...
function scheduleCommentToTaskComment(comment = {}, fallback = {}) {
  ...
  return {
    ...
    dept: comment.dept || fallback.dept || "项目成员",
...
function normalizeScheduleDepartmentFilter(filter) {
  const text = String(filter || "").trim();
  if (!text) return "全部";
  if (text === "全部") return "全部";
  if (SCHEDULE_DEPARTMENT_FILTER_LABELS.has(text)) return text;
  return normalizeScheduleModuleKey(text) || text;
}
...
function applyScheduleState(store, schedule, source = "api") {
  ...
  store.scheduleUi = {
    ...
    departmentFilter: normalizeScheduleDepartmentFilter(store.scheduleUi?.departmentFilter || "全部"),
...
setScheduleDepartmentFilter(filter) {
  const normalizedFilter = normalizeScheduleDepartmentFilter(filter);
  if (!SCHEDULE_DEPARTMENT_FILTERS.includes(normalizedFilter)) return false;
  this.scheduleUi.departmentFilter = normalizedFilter;
  return true;
},
