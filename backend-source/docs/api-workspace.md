...
- `PUT /templates/:templateId/shares`
- `POST /templates/:templateId/shares`
- `DELETE /templates/:templateId/shares/:userId`
- `POST /templates/:templateId/copy`
- `POST /templates/:templateId/apply`
- 兼容别名：以上路径均提供 `/workspace/templates` 同等 alias。

模板已接入 MySQL `templates` / `template_shares`。权限规则：

- `admin` 可读写全部模板。
- owner/created_by 可更新、删除、分享。
- `workspace` / `public` 模板、分享给当前用户的模板可读。
- 非授权访问返回 `403`；MySQL 不可用返回 `503`：`MySQL unavailable for template API`。

`GET /workspace/templates` 返回前端可直接消费的结构：
  ...
  "items": [
    {
      ...
      "visibility": "private",
      "locked": false,
      "template": { "tasks": [{ "title": "Kickoff" }] },
      "shares": []
    }
  ],
  "templates": [
    {
      "id": "template-group-uid",
      "title": "任务模板目录",
      "kind": "task",
      "children": ["上线任务模板"],
      "templateTasks": {
        "上线任务模板": [{ "title": "Kickoff" }]
      },
      "templateSchedules": {},
      "ownerId": "u-owner",
      "ownerName": "Owner",
