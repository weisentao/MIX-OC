...
Before any formal cleanup or reseed, create one recovery unit that contains both MySQL and the file storage root. The storage root is `backend-source/data/storage-v1`; it contains uploaded images, videos, attachments, chat text sidecars, and metadata needed to restore `storage_files` rows.
...
Compress-Archive -Path "data\storage-v1" -DestinationPath "backups\storage-v1-before-clean-$ts.zip" -Force
```

Do not run cleanup without both backups and an explicit production window. A database-only backup is incomplete after storage upload, board attachment, or chat text features are enabled.
